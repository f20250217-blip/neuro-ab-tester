...
export default function ShareCard({ analysis, mode, color }: ShareCardProps) {
  ...
  /* ── Share text — optimized for each platform ── */
  ...
  const handleCopyLink = useCallback(async () => {
    await navigator.clipboard.writeText(`I scored ${score}/100 on NeuroTest AI (${rank.label}). Decode your brain free: https://neurotest.live`);
    setCopied(true);
    ...
    if (navigator.share) {
      try {
        ...
        if (dataUrl) {
          ...
          const file = new File([blob], `neurotest-${score}.png`, { type: "image/png" });
          if (navigator.canShare?.({ files: [file] })) {
            await navigator.share({
              title: `My Brain: ${arch.name} (${score}/100)`,
              text: `My neural archetype is ${arch.name} ${arch.emoji} \u2014 ${rank.label}. Decode yours free:`,
        ...
        await navigator.share({
          title: `My Brain: ${arch.name} (${score}/100)`,
          text: `I scored ${score}/100 on NeuroTest AI (${rank.label}). My archetype: ${arch.name} ${arch.emoji}. Decode yours:`,
          url: "https://neurotest.live",
        });
      } catch {
  ...
  }, [captureCard, score, arch.name, arch.emoji, rank.label, handleDownload]);
  ...
  return (
    ...
    >
      ...
      {/* ── Primary share button (BIG) ── */}
      <button
        onClick={handleNativeShare}
        disabled={generating}
        className="w-full py-4 rounded-2xl text-white text-base font-bold flex items-center justify-center gap-2.5 transition-all active:scale-[0.98] disabled:opacity-60 mb-3"
        style={{
      ...
      <div className="grid grid-cols-4 gap-2">
        ...
        >
          <svg className="w-4.5 h-4.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4M7 10l5 5 5-5M12 15V3" /></svg>
          Save
        </button>
        ...
        <button
          ...
          className="flex flex-col items-center gap-1.5 py-3 rounded-xl bg-[#0c0c14] border border-[#1e1e30] hover:border-[#7c6cf0]/30 text-[#7a7a98] text-[10px] font-semibold transition-all active:scale-[0.95]"
        >
        ...
        <button
          ...
          className="flex flex-col items-center gap-1.5 py-3 rounded-xl bg-[#0c0c14] border border-[#1e1e30] hover:border-[#25D366]/30 text-[#7a7a98] text-[10px] font-semibold transition-all active:scale-[0.95]"
        >
          <svg className="w-4.5 h-4.5" viewBox="0 0 24 24" fill="#25D366"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" /></svg>
          WhatsApp
        </button>
        ...
        <button
          ...
          className="flex flex-col items-center gap-1.5 py-3 rounded-xl bg-[#0c0c14] border border-[#1e1e30] hover:border-[#4a4a68]/40 text-[#7a7a98] text-[10px] font-semibold transition-all active:scale-[0.95]"
        >
          {copied ? (
            <>
              <svg className="w-4.5 h-4.5 text-[#00e8b0]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><path d="M20 6L9 17l-5-5" /></svg>
              <span className="text-[#00e8b0]">Done!</span>
            </>
          ) : (
            <>
              <svg className="w-4.5 h-4.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1" /></svg>
              Copy
            </>
          )}
        </button>
